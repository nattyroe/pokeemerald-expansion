...
struct BattleWindowText
{
    ...
    union
    {
        struct
        {
            DEPRECATED("Use color.background instead")
            u8 bgColor;
            DEPRECATED("Use color.foreground instead")
            u8 fgColor;
            DEPRECATED("Use color.shadow instead")
            u8 shadowColor;
            DEPRECATED("Use color.accent instead")
            u8 accentColor;
        };
...
const u8 *const gBattleStringsTable[STRINGID_COUNT] =
    {
        ...
        [STRINGID_NOTDITTOBLOCKEDBALL] = COMPOUND_STRING("That's not a Ditto!"),
        [STRINGID_DONTBEALOSER] = COMPOUND_STRING("Don't be a loser!"),
...
void BufferStringBattle(enum StringID stringID, enum BattlerId battler)
{
    ...
    case STRINGID_RETURNMON:                                           // sending poke to ball msg
        if ((GetBattlerPosition(battler) & BIT_FLANK) == B_FLANK_LEFT) // battler 0 and 1
        {
            ...
            else if (TRAINER_BATTLE_PARAM.opponentA == TRAINER_LINK_OPPONENT || gBattleTypeFlags & BATTLE_TYPE_RECORDED_LINK) // Link Opponent A and test opponent
            {
